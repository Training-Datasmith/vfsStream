 ...
 * @since  1.3.0
 */
class Large_File_Content extends Seekable_File_Content implements File_Content
{
    /**
     ...
     */
    public static function with_kilobytes(int $kilobyte): self
    {
     ...
     */
    public static function with_megabytes(int $megabyte): self
    {
        return self::with_kilobytes($megabyte * 1024);
    }
    /**
     * create large file with given size in gigabyte
     */
    public static function with_gigabytes(int $gigabyte): self
    {
        return self::with_megabytes($gigabyte * 1024);
    }
    /**
    ...
    {
        return $this->do_read(0, $this->size);
    }
    /**
     ...
     */
    protected function do_read(int $offset, int $count): string
    {
     ...
     */
    protected function do_write(string $data, int $offset, int $length): void
    {
        for ($i = 0; $i < $length; $i++) {
    ...
    {
        $this->size = $size;
        foreach (array_filter(array_keys($this->content), static function ($pos) use ($size): bool {
            return $pos >= $size;
        }) as $remove_pos) {
            unset($this->content[$remove_pos]);
        }
        return true;
    }
}
